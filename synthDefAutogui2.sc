...
SynthDefAutogui2 {
	...
	*new { arg name, aSynth, rate = \audio, target,args,addAction=\addToTail, 
			...
			specs, onInit = true, fscale=1 ;
			
		^super.new.initSynthDefAutogui
			([name, aSynth, rate, target,args,addAction, 
			closeOnCmdPeriod, freeOnClose , window, step , hOff , vOff, scopeOn, specs, onInit, fscale]) 
	}

	initSynthDefAutogui { arg argArray ;	
		#name, aSynth, rate, target, args, addAction, 
			closeOnCmdPeriod, freeOnClose, window, step , hOff, vOff, scopeOn, specs, onInit, fscale  = argArray ;
	...
	autogui {
		...
		var hMod = h + mrg ;
		var xOff = wMod*2.5 ;
		var fSize = 12/50*step*fscale ; // for font scaling
