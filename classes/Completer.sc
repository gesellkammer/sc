Completer {
	...
	*initClass {
		StartUp.add {
			...
			annotatedMethods = IdentityDictionary[
				...
				( {|x| x.beginsWith("is") or: {x.beginsWith("could")} } -> Boolean),
				({|x| #["reverse", 
						"stutter",
						"flop",
						"rotate",
	...
	*getArgs {|class, method|
		...
		//^class.methodCallString(method.asSymbol);
	...
	*infereClass {|str|
		...
		if( prev_char.notNil ) {
			...
			*/
			var str2 = str.trim;
			var last2chars = str2.[str2.size - 2..];
			
			if( last2chars == "]." ) { ^Array };
			if( last2chars == "}." ) { ^Function };
		};
...
+ Class { 
    methodCallString {|methodName| 
        var m = this.findRespondingMethodFor(methodName.asSymbol); 
        var res = methodName.asString++" ("; 
        var value; 
        m ?? {^nil}; 
        m.argNames.do {|a,i| 
